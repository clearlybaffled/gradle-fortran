...
class IntelFortranPlatformToolProvider extends AbstractPlatformToolProvider {
    ...
    IntelFortranPlatformToolProvider(BuildOperationExecutor buildOperationExecutor, OperatingSystemInternal targetOperatingSystem, ToolSearchPath toolSearchPath, ToolRegistry toolRegistry, ExecActionFactory execActionFactory, CompilerOutputFileNamingSchemeFactory compilerOutputFileNamingSchemeFactory, boolean useCommandFile, WorkerLeaseService workerLeaseService, CompilerMetaDataProvider<GccMetadata> metadataProvider) {
    ...
    public CommandLineToolSearchResult locateTool(ToolType compilerType) {
        if (compilerType == ToolType.C_COMPILER || compilerType == ToolType.LINKER) {
            return toolSearchPath.locate(compilerType, "ifort")
        }
    ...
    protected Compiler<LinkerSpec> createLinker() {
        ...
        versionAwareCompiler(new IntelFortranLinker(buildOperationExecutor, commandLineTool(linkerTool), context(linkerTool), useCommandFile, workerLeaseService), ToolType.LINKER)
    }
    ...
    protected Compiler<FortranCompileSpec> createFortranCompiler() {
        ...
        IntelFortranCompiler compiler = new IntelFortranCompiler(buildOperationExecutor, compilerOutputFileNamingSchemeFactory, commandLineTool(compilerTool), context(compilerTool), getObjectFileExtension(), useCommandFile, workerLeaseService)
